  ...
  useEffect(() => {
    ...
    es.onmessage = (e) => {
      ...
      if (data.platform && data.promptText) {
        ...
        setLogs(prev => [{
          id: Date.now(),
          platform: PLATFORMS.find(p => p.key === data.platform)?.label || data.platform,
    ...
    es.onerror = () => {
      ...
      const poll = setInterval(async () => {
        try {
          ...
          setProgress({ status: data.status, completed: data.completed_queries, total: data.total_queries, pct: data.total_queries > 0 ? Math.round((data.completed_queries/data.total_queries)*100) : 0 });
          if (data.status === 'completed') {
            clearInterval(poll);
            setTimeout(() => nav(`/report/${projectId}`), 1500);
          }
      ...
      <div className="max-w-2xl mx-auto px-4 py-10">
        ...
        <div className="card">
          <div className="flex items-start justify-between mb-5">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                Checking all prompts across 5 platforms — each runs 5× for consistency scoring.
              </p>
            </div>
            {isCompleted && (
              <span className="flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full" style={{background:'#E1F5EE',color:'#0F6E56'}}>
          ...
          <div className="grid grid-cols-5 gap-2 mb-5">
            {PLATFORMS.map(p => (
              <div key={p.key} className="border rounded-lg px-2 py-2.5 flex flex-col items-center gap-1.5 text-center" style={{borderWidth:'0.5px',borderColor:'#e8e6df'}}>
                <div className="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-medium flex-shrink-0" style={{background:p.bg,color:p.color}}>{p.abbr}</div>
                ...
                <span className="text-[10px] font-medium" style={{color:'#7F77DD'}}>
          ...
          <div ref={logsRef} className="space-y-1.5 mb-4 max-h-48 overflow-hidden">
            ...
            {logs.map(log => (
              <div key={log.id} className="flex items-center gap-2.5 text-xs px-3 py-2 rounded-lg" style={{background: log.tier==='absent'?'#FCEBEB': log.tier==='primary'||log.tier==='top'?'#E1F5EE':'#FAEEDA', color: log.tier==='absent'?'#791F1F': log.tier==='primary'||log.tier==='top'?'#085041':'#854F0B'}}>
          ...
          <div>
            ...
            <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
              <div className="h-full rounded-full transition-all duration-500" style={{width:`${progress.pct}%`,background:'#7F77DD'}} />
            </div>
          </div>
          ...
          <div className="flex items-center justify-between mt-6 pt-5 border-t" style={{borderTopWidth:'0.5px',borderColor:'#e8e6df'}}>
            <span className="text-xs text-gray-400">
              {isCompleted ? 'Redirecting to report...' : `Est. ${Math.max(0, Math.round(((progress.total - progress.completed) * 0.8)))}s remaining`}
            </span>
            <button className="btn-primary flex items-center gap-2"
              onClick={() => nav(`/report/${projectId}`)}>
